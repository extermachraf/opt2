-- =====================================================================================
-- FIX: Remove subquery from index predicate to resolve SQL error 0A000
-- PostgreSQL does not allow subqueries in index WHERE clauses
-- =====================================================================================

-- First, let's get the MUST template ID and ensure questionnaire structure is correct
DO $$
DECLARE
...
BEGIN
    ...
    IF must_template_uuid IS NULL THEN
        ...
        RAISE NOTICE 'Created new MUST template: %', must_template_uuid;
    END IF;
...
-- =====================================================================================
-- STEP 7: Create fixed index WITHOUT subquery (FIXED: removes error 0A000)
-- Create a simple index on template_id and order_index without WHERE clause
-- =====================================================================================

-- Drop any existing problematic index first
DROP INDEX IF EXISTS idx_questionnaire_questions_must_template;

-- Create simple index without subquery to avoid error 0A000
CREATE INDEX IF NOT EXISTS idx_questionnaire_questions_template_order 
ON questionnaire_questions(template_id, order_index);

-- Create additional index for MUST-specific optimization if needed
-- (This is optional and can be created later if performance requires it)
-- CREATE INDEX IF NOT EXISTS idx_questionnaire_questions_must_type
-- ON questionnaire_questions(template_id) 
-- WHERE questionnaire_type = 'must'::questionnaire_type;
-- Note: Above is commented because we don't have direct questionnaire_type in this table
...
BEGIN
    ...
    RAISE NOTICE 'Index Error: FIXED - No more subquery in index predicate';
    RAISE NOTICE '=======================================';
END $$;
...
BEGIN
    ...
    RAISE NOTICE '✅ Index error 0A000 FIXED - removed subquery from WHERE clause';
